...
static void *callback_mcode_init(global_State *g, uint32_t *page)
{
  ...
  uintptr_t ug = (uintptr_t)(void *)g;
  MSize slot;
  *p++ = RISCVI_LUI | RISCVF_D(RID_X6) | (ug & 0xfffff000);
  *p++ = RISCVI_LUI | RISCVF_D(RID_X5) | ((target << 12) & 0xfffff000);
  *p++ = RISCVI_ORI | RISCVF_D(RID_X6) | RISCVF_RS1(RID_X6) | (ug << 20);
  *p++ = RISCVI_SRLI | RISCVF_D(RID_X5) | RISCVF_RS1(RID_X5) | (12 << 20);
  *p++ = RISCVI_JALR | RISCVF_D(RID_X0) | RISCVF_RS1(RID_X5);

  for (slot = 0; slot < CALLBACK_MAX_SLOT; slot++) {
    *p++ = RISCVI_ADDI | RISCVF_D(RID_X7) | (slot << 20);
    uint32_t n = (page-p)*4;
...
#if !LJ_ABI_SOFTFP	/* RISCV64 hard-float */
#define CALLBACK_HANDLE_REGARG \
  if (isfp && nfpr < CCALL_NARG_FPR) {  /* Try to pass argument in FPRs. */ \
    sp = &cts->cb.fpr[nfpr]; \
    nfpr += (isfp == 3) ? 2 : 1; \
